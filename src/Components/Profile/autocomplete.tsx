...
export default function Autocomplete({
  ...
  createTest?: (name: string, heading?: string) => Promise<void>;
  ...
  className?: string;
  comparator?: string;
  handleChange?: (name: string, query: string, comparator: string) => void;
  heading?: string;
}) {
  const [query, setQuery] = useState('');

  console.log(tests, 'tests', query, 'query');
    ...
    <div className={`flex  items-center  mx-3 my-3   ${className}`}>
      <div className="w-full">
        <Combobox
          ...
          onChange={(name) => {
            if (handleChange) {
              if (name) handleChange(name, query, comparator);
              else {
                if (createTest) createTest(query, heading);
              }
            } else {
              if (name) {
            ...
            >
              <Combobox.Options className="absolute mt-1 max-h-60 w-full overflow-auto  bg-white  py-1  text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm z-10">
                {filteredTests.length === 0 && createTest && (
                  <Combobox.Option value={undefined} className="p-3 cursor-pointer bg-teal-600 text-white">
                    Create "{query}"
                  </Combobox.Option>
                )}
                        ...
                        <>
                          <span
                            className={`block truncate cursor-pointer ${
                              testIds.find((t) => t[comparator] === person.name) ? 'font-medium' : 'font-normal'
                            }`}
                          >
                            {person.name}
                          </span>
